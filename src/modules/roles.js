...
module.exports = function(bot) {
				...
				.then(result => {
					...
					.then(doneMsg => {
						settings.getMultiple(guild.id, ['roles.autoDeleteMessages', 'roles.autoDeleteOtherMessages', 'roles.autoDeleteResponseDelay']).then(values => {
							...
							const selfDeleteDelay = parseInt(values['roles.autoDeleteResponseDelay'], 10) * 1000;
							setTimeout(() => bot.deleteMessage(msg.channel.id, doneMsg.id), selfDeleteDelay);
						});
					});
				});
		});
	...
	// (ADMIN) Setting assignable roles
